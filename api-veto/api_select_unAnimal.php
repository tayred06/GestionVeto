...
// Requête SQL 
$requete_sql = "SELECT A.id AS idA, A.prenom AS prenomA, A.type AS typeA, A.dateNaiss AS dateNaissA, A.linkImg AS linkImgA, C.prenom AS prenomC, C.nom AS nomC FROM animal A JOIN client C ON A.idProprio=C.id WHERE A.id = $recherche;";
...
if (mysqli_connect_errno()) { 
    ...
    $donnees = '[';
	while ($animal = mysqli_fetch_assoc($resultat)) {

        $donnees = $donnees . '{';
        $donnees = $donnees . '"id": "' . $animal['idA'] . '" ,';
        $donnees = $donnees . '"prenom": "' . $animal['prenomA'] . '" ,';
        $donnees = $donnees . '"type": "' . $animal['typeA'] . '" ,';
        $donnees = $donnees . '"dateNaiss": "' . $animal['dateNaissA'] . '",';
        $donnees = $donnees . '"nomProprio": "' . $animal['prenomC'] . '",';
        $donnees = $donnees . '"prenomProprio": "' . $animal['nomC'] . '",';
        $donnees = $donnees . '"linkImg": "' . $animal['linkImgA'] . '"';
        $donnees = $donnees . '},';
