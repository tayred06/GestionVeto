...
// Requête SQL 
$requete_sql = "SELECT id, prenom, type, dateNaiss FROM animal WHERE idProprio = $recherche;";
...
if (mysqli_connect_errno()) { 
	...
	while ($animal = mysqli_fetch_assoc($resultat)) {
        ...
        $donnees = $donnees . '{';
        $donnees = $donnees . '"id": "' . $animal['id'] . '" ,';
        $donnees = $donnees . '"prenom": "' . $animal['prenom'] . '" ,';
        $donnees = $donnees . '"type": "' . $animal['type'] . '" ,';
        $donnees = $donnees . '"dateNaiss": "' . $animal['dateNaiss'] . '"';
        $donnees = $donnees . '},';
