...
// Requête SQL 
$requete_sql = "SELECT `id`, `type`, `date`, `duree`, `commentaire` FROM `intervention` WHERE id = $recherche;";
...
if (mysqli_connect_errno()) { 
    ...
    $donnees = '[';
	while ($inter = mysqli_fetch_assoc($resultat)) {

        $donnees = $donnees . '{';
        $donnees = $donnees . '"id": "' . $inter['id'] . '" ,';
        $donnees = $donnees . '"type": "' . $inter['type'] . '" ,';
        $donnees = $donnees . '"date": "' . $inter['date'] . '" ,';
        $donnees = $donnees . '"duree": "' . $inter['duree'] . '",';
        $donnees = $donnees . '"commentaire": "' . $inter['commentaire'] . '"';
        $donnees = $donnees . '},';
